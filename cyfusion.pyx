...
cdef class PyViews:
  ...
  def __init__(self, float[:,:,::1] depthmaps, float[:,:,::1] Ks, float[:,:,::1] Rs, float[:,::1] Ts, float[:,:,::1] Ks_inv, float[:,:,::1] cam2world):
...
#def color_tsdf_cpu(PyViews views, np.ndarray np_color_kernel, int depth, int height, int width, float vx_size, float truncation, bool unknown_is_free, int n_threads=8, float box_size=0.5):
...
#  color_fusion_tsdf_cpu(views.views, vx_size, truncation, unknown_is_free, n_threads, py_vol.vol)
#  return vol, rgb_vol

def tsdf_cpu(PyViews views, int depth, int height, int width, float vx_size, float truncation, bool unknown_is_free, int n_threads=8, float box_size=0.5):
  vol = np.empty((depth, height, width), dtype=np.float32)
  cdef float[:,:,::1] vol_view = vol
  cdef float volume_box_size = box_size
  cdef PyVolume py_vol = PyVolume(volume_box_size, vol_view)
